...
class GemmaRotaryEmbedding(nn.Module):
    ...
    def forward(self, x, position_ids, seq_len = None):
        ...
        # Copy the inv_freq tensor for bqtch in the sequence
        # inv_freq_expanded [bz, 1, seq_len]
        ...
        with torch.autocast(device_type=device_type, enabled=False):
            ...
            # freqs: [bz, head_dim // 2, 1] @ [bz, 1, seq_len] --> [bz, seq_len, head_dim//2]
            freqs = (inv_freq_expanded.float() @ position_ids_expanded.float()).transpose(1, 2)
            # emb: [bz, seq_len, head_dim]
            emb = torch.cat((freqs, freqs), dim=-1)
            # cos, sin: [bz, seq_len, head_dim]
            cos = emb.cos()
            sin = emb.sin()
        return cos.to(dtype=x.dtype), sin.to(dtype=x.dtype)

def rotate_half(x):
    # build the [-x2, x1, -x4, x3,...] tensor for the sin part of positional encoding
    x1 = x[..., : x.shape[-1] // 2]
    x2 = x[..., x.shape[-1] // 2 :]
